            ...
            <div className="container mx-auto pt-8 pb-4 h-[calc(100vh-120px)]">
                {user?.isAdmin ?
                    <form onSubmit={sendImage} className="" action={`${url}/product/create`} method="post" encType="multipart/form-data">
                        <input type='name' placeholder="name" className="border-2 border-gray-700 rounded-lg w-48 sm:w-60 pl-2 ml-10 py-1" onChange={(e) => setTitle(e.target.value)} value={title}></input>
                        <br />
                        <hr />
                        <br />
                        <input type="number" placeholder="price" className="pl-2 py-1 ml-10 rounded-lg" onChange={(e) => setPrice(e.target.value)} value={price} ></input>
                        <br />
                        <hr />
                        <br />
                        <select placeholder="category" className="py-1 pl-2 ml-10 rounded-lg" onChange={(e) => setCategory(e.target.value)} value={category} >
